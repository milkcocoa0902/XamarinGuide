...
はい，いきなりサブタイ回収しました．@<br>{}
Xamarinっていうのは，Androidなら@<tt>{Java}もしくは@<tt>{Kotlin}のコードを，iOSなら@<tt>{Swift}のコードをラッピングして@<tt>{C#}を用いて開発することのできるフレームワークです.@<br>{}
...
とりあえず実際にアプリをいくつか作っていきながらXamarinでの開発に慣れていきましょうね．@<br>{}
そうそう，今回はAndroidアプリに関してのみ触れたいと思います．



そうそう，本著ではXamarinの導入方法とかプロジェクトの作成方法は解説しないことにします．@<br>{}
めんどくさいんだもん．
...
まず手始めに定番とも言えるキッチンタイマーを作ってみましょう．@<br>{}
...
まずは，アプリのレイアウトを決めましょう．プロジェクトを作成した時に@<tt>{activity_main.xml}というファイルが作成されると思うので，そちらをいじっていきましょう．@<br>{}
ここはシンプルに@<tt>{TextView}と@<tt>{Button}でいこうではありませんか. 


//listnum[][activity_main.xml][xml]{
...
最低限キッチンタイマー的な動作をするプログラムを書いて見ましょう．．@<br>{}
先ほどの要件定義を満たすために，カウントダウン機構とスタート動作，ビープ音の生成・発音機能を実装していくことにします．


//listnum[][MainActivity.cs][CS]{
using Android.App;
using Android.Media;
using Android.OS;
...
@<strong>{(1)}, @<strong>{(2)} ではそれぞれのコントロールを取得してます．@<br>{}
両者を比べてみれば一目瞭然ですが，@<strong>{(1)} ではインスタンスを生成しているのに対して @<strong>{(2)} では，生成せずに直接操作をしてます．@<br>{}
別に，以降もそのコントロールを使用するなら保存しておけばいいですし，その場限りでしか操作しないなら直接構築してあげればいいかなと思います．@<br>{}
そしてそして，@<strong>{Xamarinでは，コントロールのアクションはラムダ式で登録できる} のです！！！@<br>{}
あと，二重，三重にボタンを押してしまわないようにスタートしたらクリックできないようにしてしまおう．  



@<strong>{(3)} では，カウント終了時に鳴らす音声を生成しています．@<br>{}
思いっきり高校物理の波動の分野ですね．覚えてますか？？笑  



@<strong>{(4)} の@<tt>{Action}という関数は，ボタンクリック時に1000[ms]遅延で実行するタスク，つまりタイマーカウントを担っています，遅延動作をネストしたかったのでわざわざ関数化しました．  



@<strong>{(5)} の部分は，タイマーの残り時間をフォアグラウンドで実行するためのコードです．@<tt>{RunOnUiThread()}を用いる事で，UIの更新をスレッドセーフ(?)に行うことができるのです．  



さて，これだけで本当に簡単なキッチンタイマーができてしまいました．@<br>{}
しかしながらこれでは，スタートしてカウント終了したらそれっきりです．これでは使い物になりませんよね．@<br>{}
...
とりあえず，タイマーが終了したら再度使えるようにリセットしたいのでリセット機能を追加してみましょう．@<br>{}
まずはレイアウトをいじります．
...
ここで，キモかどうかはちょっと微妙ですが，ボタンとボタンの間を画面の中心に合わせたいので@<tt>{RelativeLayout}でラッピングしています.
...
@<strong>{(1)} では，先ほどの時点では保持していなかったstartボタンへのコントロールを保持するようにしています．というのも，リセット時にstartボタンをクリック可能にするためにアクセスする必要があり，その度にコントロールを獲得していたら効率が悪いからです．  



対して @<strong>{(2)} ではリセットボタンのコントロールを保持していません．別にリセットボタンは他のボタンとかに左右されないので．  



@<strong>{(3)} では，startボタンで再帰的に登録されたカウントダウンタイマーの登録解除をしています．  



これだけの変更で，タイマーをリセットすることができました．@<br>{}
簡単ですね．
...
次に加えたい機能は，計測時間を任意のものにすることです．@<br>{}
現在は3分決め打ちなのでラーメンタイマーにしかなりません，別にそれでもいいって人もいるでしょうが，ちょっと用途が限定的すぎますね．  



時間を表示させる部分のUIを以下のように2個の@<tt>{EditText}に分解することにしました．  
...
少し長めに見えるかもしれないけどあまり大したことはしていません．@<br>{}
時分を入力するための@<tt>{TextEdit}は，数値以外の入力を受け付けないようにしたり，文字数を2桁までに制限したりしてプログラムの負担を減らせるようにしていきましょう．  
...
今回，時間設定を分と秒に分解したのでそれぞれ保持するための変数を用意しておき，そこに設定値を読み込むことにします．@<strong>{(1)}
